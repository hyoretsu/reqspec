import { useState } from "react";
import { useHotkeys } from "react-hotkeys-hook";
import { WebSocketPanel } from "@/components/protocols";
import { AuthTab } from "@/components/request/RequestBuilder/AuthTab";
import { BodyTab } from "@/components/request/RequestBuilder/BodyTab";
import { DocsTab } from "@/components/request/RequestBuilder/DocsTab";
import { HeadersTab } from "@/components/request/RequestBuilder/HeadersTab";
import { ParamsTab } from "@/components/request/RequestBuilder/ParamsTab";
import { ScriptsTab } from "@/components/request/RequestBuilder/ScriptsTab";
import type { RequestSection } from "@/components/request/RequestBuilder/types";
import { UrlBar } from "@/components/request/RequestBuilder/UrlBar";
import {
	Button,
	CustomSelect,
	MethodBadge,
	type SelectOption,
	Spinner,
	type TabItem,
	Tabs,
} from "@/components/ui";
import { saveDraft } from "@/hooks/queries/use-requests";
import { useSendRequest } from "@/hooks/use-send-request";
import {
	HTTP_METHODS,
	type HttpMethod,
	type ProtocolKind,
	type WebSocketConfig,
} from "@/lib/request/model";
import { useActiveRequestStore } from "@/lib/store/active-request.store";
import { useTabsStore } from "@/lib/store/tabs.store";

const METHOD_OPTIONS: SelectOption<HttpMethod>[] = HTTP_METHODS.map(
	(method) => ({
		label: method,
		value: method,
		adornment: <MethodBadge method={method} />,
	}),
);

const PROTOCOL_OPTIONS: SelectOption<ProtocolKind>[] = [
	{ label: "HTTP", value: "http" },
	{ label: "WebSocket", value: "websocket" },
	{ label: "Socket.IO", value: "socketio" },
	{ label: "MQTT", value: "mqtt" },
	{ label: "gRPC", value: "grpc" },
	{ label: "GraphQL", value: "graphql" },
];

const PROTOCOL_LABEL = Object.fromEntries(
	PROTOCOL_OPTIONS.map((o) => [o.value, o.label]),
) as Record<ProtocolKind, string>;

const EMPTY_WS_CONFIG: WebSocketConfig = {
	protocols: [],
	messageFormat: "text",
	draft: "",
};

function enabledCount(items: { enabled: boolean; key: string }[]): number {
	return items.filter((i) => i.enabled && i.key !== "").length;
}

export function RequestBuilder() {
	const [section, setSection] = useState<RequestSection>("params");
	const draft = useActiveRequestStore((state) => state.draft);
	const requestId = useActiveRequestStore((state) => state.requestId);
	const dirty = useActiveRequestStore((state) => state.dirty);
	const isSending = useActiveRequestStore((state) => state.isSending);
	const patchDraft = useActiveRequestStore((state) => state.patchDraft);
	const markSaved = useActiveRequestStore((state) => state.markSaved);
	const send = useSendRequest();
	const openScratch = useTabsStore((state) => state.openScratch);
	const persistActive = useTabsStore((state) => state.persistActive);

	const tabs: TabItem<RequestSection>[] = [
		{ id: "params", label: "Params", badge: badge(enabledCount(draft.params)) },
		{
			id: "headers",
			label: "Headers",
			badge: badge(enabledCount(draft.headers)),
		},
		{
			id: "body",
			label: "Body",
			badge: draft.body.type !== "none" ? dot() : undefined,
		},
		{
			id: "auth",
			label: "Auth",
			badge: draft.auth.type !== "none" ? dot() : undefined,
		},
		{
			id: "scripts",
			label: "Scripts",
			badge: draft.events.length > 0 ? dot() : undefined,
		},
		{ id: "docs", label: "Docs" },
	];

	const protocol: ProtocolKind = draft.protocol ?? "http";

	const onSave = async () => {
		if (!requestId) return;
		await saveDraft(requestId, { request: draft });
		markSaved();
		persistActive();
	};

	// Switching protocol seeds the protocol-specific payload when it is first selected.
	const onProtocol = (next: ProtocolKind) =>
		patchDraft(
			next === "websocket" && !draft.websocket
				? { protocol: next, websocket: EMPTY_WS_CONFIG }
				: { protocol: next },
		);

	useHotkeys(
		"mod+enter",
		(e) => {
			e.preventDefault();
			if (protocol === "http" && !isSending && draft.url.trim() !== "")
				void send();
		},
		{ enableOnFormTags: true },
	);
	useHotkeys(
		"mod+s",
		(e) => {
			e.preventDefault();
			void onSave();
		},
		{ enableOnFormTags: true },
	);
	useHotkeys(
		"mod+t",
		(e) => {
			e.preventDefault();
			openScratch();
		},
		{ enableOnFormTags: true },
	);

	const protocolSelect = (
		<CustomSelect
			aria-label="Protocol"
			value={protocol}
			options={PROTOCOL_OPTIONS}
			onChange={onProtocol}
			className="w-32 shrink-0"
		/>
	);
	const saveButton = requestId ? (
		<Button variant="secondary" onClick={onSave} disabled={!dirty}>
			Save
		</Button>
	) : null;

	if (protocol === "websocket") {
		return (
			<div className="flex h-full flex-col">
				<div className="flex items-center gap-2 p-3 pb-0">
					{protocolSelect}
					<div className="flex-1" />
					{saveButton}
				</div>
				<div className="min-h-0 flex-1">
					<WebSocketPanel />
				</div>
			</div>
		);
	}

	if (protocol !== "http") {
		return (
			<div className="flex h-full flex-col">
				<div className="flex items-center gap-2 p-3 pb-0">
					{protocolSelect}
					<div className="flex-1" />
					{saveButton}
				</div>
				<div className="flex flex-1 items-center justify-center p-6 text-center text-sm text-muted">
					{PROTOCOL_LABEL[protocol]} support is coming in a later milestone.
				</div>
			</div>
		);
	}

	return (
		<div className="flex h-full flex-col gap-3 p-3">
			<div className="flex flex-col gap-2 sm:flex-row">
				<div className="flex gap-2">
					{protocolSelect}
					<CustomSelect
						aria-label="HTTP method"
						value={draft.method}
						options={METHOD_OPTIONS}
						onChange={(method) => patchDraft({ method })}
						className="w-32 shrink-0"
					/>
					<UrlBar className="flex-1" />
				</div>
				<div className="flex gap-2">
					<Button
						onClick={send}
						disabled={isSending || draft.url.trim() === ""}
						className="flex-1 sm:flex-none"
					>
						{isSending ? <Spinner /> : "Send"}
					</Button>
					{saveButton}
				</div>
			</div>

			<Tabs tabs={tabs} value={section} onChange={setSection} />

			<div className="min-h-0 flex-1 overflow-auto">
				{section === "params" ? <ParamsTab /> : null}
				{section === "headers" ? <HeadersTab /> : null}
				{section === "body" ? <BodyTab /> : null}
				{section === "auth" ? <AuthTab /> : null}
				{section === "scripts" ? <ScriptsTab /> : null}
				{section === "docs" ? <DocsTab /> : null}
			</div>
		</div>
	);
}

function badge(count: number) {
	if (count === 0) return undefined;
	return (
		<span className="rounded-full bg-surface px-1.5 text-[10px] font-semibold text-muted">
			{count}
		</span>
	);
}

function dot() {
	return <span className="h-1.5 w-1.5 rounded-full bg-primary" />;
}
