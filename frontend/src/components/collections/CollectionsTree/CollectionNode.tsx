...
export function CollectionNode({ collection }: { collection: CollectionRow }) {
	...
	const openRunner = useRunnerStore((state) => state.open);
	...
	const renameCollection = async () => {
		...
		if (name)
			await collectionMutations.rename.mutateAsync({ id: collection.id, name });
	};
	...
	const renameRequest = async (row: RequestRow) => {
		...
		if (name)
			await requestMutations.update.mutateAsync({
				id: row.id,
				patch: { name },
			});
	};
	...
	const renderRequest = (
		row: RequestRow,
		handleProps?: Parameters<typeof RequestNode>[0]["handleProps"],
	) => (
		<RequestNode
			row={row}
					...
					{folders?.map((folder) => {
						...
						return (
							<div key={folder.id}>
									...
									<div className="flex opacity-0 group-hover:opacity-100">
										...
										<IconButton
											...
											onClick={async () => {
												...
												if (name)
													await folderMutations.rename.mutateAsync({
														id: folder.id,
														name,
													});
											}}
										...
										<IconButton
											label="Delete folder"
											onClick={() => folderMutations.remove.mutate(folder.id)}
										>
											✕
										</IconButton>
									</div>
								...
								<div className="pl-3">
									{folderRequests.map((row) => renderRequest(row))}
								</div>
							</div>
						);
					})}
