...
export function useRequestMutations(collectionId: string) {
	...
	const create = useMutation({
		mutationFn: ({
			...
			requestsRepo.createRequest(collectionId, name, folderId ?? null, request),
	...
	const update = useMutation({
		mutationFn: ({
			...
			id: string;
			patch: Partial<Pick<RequestRow, "name" | "request">>;
		}) => requestsRepo.updateRequest(id, patch),
		onSuccess: invalidate,
	});

	const duplicate = useMutation({
		mutationFn: requestsRepo.duplicateRequest,
		onSuccess: invalidate,
	});
	const reorder = useMutation({
		mutationFn: requestsRepo.reorderRequests,
		onSuccess: invalidate,
	});
	const remove = useMutation({
		mutationFn: requestsRepo.deleteRequest,
		onSuccess: invalidate,
	});
